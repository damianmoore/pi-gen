...
class timelapse_config(object):
    ...
    def __init__(self, config_map={}):
        self.w = config_map.get('w', 3280)
        self.h = config_map.get('h', 2464)
        self.iso = config_map.get('iso', 100)
        self.interval = config_map.get('interval', 30)
        ...
        # Setting the maxss under one second (1000000) prevents flipping into a slower camera mode.
        self.maxss = config_map.get('maxss', 2000000)
...
class timelapse(object):
    ...
    def dynamic_adjust(self, config=None, state=None):
        ...
        def adj(v):
            return v * (1.0 + 1.0 * delta * config.gamma / config.targetBrightness)
    ...
    def findinitialparams(self, config=None, state=None):
        ...
        while abs(config.targetBrightness - state.brData[-1]) > 4:
            ...
            print ('ss: % 10d\tx: % 6.4f br: % 4d\t' % (state.currentss, round(state.xData[-1], 4), round(state.brData[-1], 4)))
            if state.xData[-1] >= 1.0:
                if killtoken is True:
                    break
                else:
                    killtoken = True
    ...
    def run_timelapse(self, config=None, state=None):
        ...
        while ((elapsed < config.maxtime or config.maxtime == -1) and (state.shots_taken < config.maxshots or config.maxshots == -1)):
            loopstart = time.time()
...
def main(argv):
        ...
        parser.add_argument('-i', '--interval', default=30, type=int, help='Set photo interval in seconds.    \nRecommended miniumum is 6.')
